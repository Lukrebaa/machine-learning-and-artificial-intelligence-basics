% BFS
function [best_cost, best_path] = bfs(distance_matrix, max_iterations)
    n = size(distance_matrix, 1);
    start_city = 1;
    queue = {struct('current_city', start_city, 'path', start_city, 'distance', 0)};
    best_cost = Inf;
    best_path = [];
    iterations = 0;
    ...
    while ~isempty(queue) && iterations < max_iterations
        ...
        for next_city = 1:n
            if ~ismember(next_city, current.path)
                ...
                queue{end+1} = struct('current_city', next_city, 'path', new_path, 'distance', new_distance);
            end
        end
